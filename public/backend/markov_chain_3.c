...
void update_transition_count(TransitionList* transition_list, Token* previous1, Token* previous2, Token* source, Token* destination) {
...
void train_model(TransitionList* transition_list, TokenList* token_list, Token* dummy, Token* end_token) {    
    ...
    //Token *dummy1 = create_token("", 1);
    //Token *dummy2 = create_token("", 1);
    Token *previous1 = dummy; //dummy1
    Token *previous2 = dummy; //dummy2
    Token *current = dummy;
    ...
    for (int i = 0; i < token_list->length; i++) {
        ...
        if (previous2 != NULL) {
            ...
            update_transition_count(transition_list, previous1, previous2, current, destination);
        }
    ...
    update_transition_count(transition_list, previous1, previous2, current, end_token);
}
...
void save_transition_list(TransitionList* transition_list, const char* filename) {
    ...
    for (int i = 0; i < transition_list->length; i++) {
        ...
        int length = strlen(transition->previous2->name) + 1;
...
Token* generate_next_token(TransitionList* transition_list, Token* previous1, Token* previous2, Token* source) {
...
Token** generate_tokens(TransitionList* transition_list, Token* previous1, Token* previous2, Token* source) {
    Token** tokens = (Token**) malloc(6 * sizeof(Token*));

    tokens[0] = generate_next_token(transition_list, previous1, previous2, source);
    tokens[1] = generate_next_token(transition_list, previous2, source, tokens[0]);
    tokens[2] = generate_next_token(transition_list, source, tokens[0], tokens[1]);
    tokens[3] = generate_next_token(transition_list, tokens[0], tokens[1], tokens[2]);
    tokens[4] = generate_next_token(transition_list, tokens[1], tokens[2], tokens[3]);
    tokens[5] = generate_next_token(transition_list, tokens[2], tokens[3], tokens[4]);
...
int main(int argc, char const *argv[]) {
    ...
    save_transition_list(transition_list, "model_3.bin");
    ...
    // Token** tokens = generate_tokens(transition_list, token_first, token_second, token_third);
    // printf("for ( int ");
    // for(int i = 0; i < 6; i++) {
