...
typedef struct {
    ...
    // 1 if this token represents the end of a line, 0 otherwise
    // Not really used besides debugging but if I implemented this further then
    // I could use it to do newlines in code suggestions.
    int is_end; 
} Token;
...
void update_transition_count(TransitionList* transition_list, Token* previous, Token* source, Token* destination) {
    ...
    new_transition->previous = previous;
...
void train_model(TransitionList* transition_list, TokenList* token_list, Token* dummy, Token* end_token) {    
    ...
    for (int i = 0; i < token_list->length; i++) {
        ...
        if (previous != NULL) {
            // Update the transition count from previous1 and previous2 to current
            update_transition_count(transition_list, previous, current, destination);
        }
    ...
    update_transition_count(transition_list, previous, current, end_token);
    //printf("HERE!\n");
}
...
void save_transition_list(TransitionList* transition_list, const char* filename) {
    ...
    for (int i = 0; i < transition_list->length; i++) {
        ...
        int length = strlen(transition->previous->name) + 1;
        fwrite(&length, sizeof(int), 1, fp);
        fwrite(transition->previous->name, sizeof(char), length, fp);
...
Token* generate_next_token(TransitionList* transition_list, Token* previous, Token* source) {
...
Token** generate_tokens(TransitionList* transition_list, Token* previous, Token* source) {
    Token** tokens = (Token**) malloc(4 * sizeof(Token*));

    tokens[0] = generate_next_token(transition_list, previous, source);
    tokens[1] = generate_next_token(transition_list, source, tokens[0]);
    tokens[2] = generate_next_token(transition_list, tokens[0], tokens[1]);
...
int main(int argc, char const *argv[]) {
    ...
    // Token *ret = generate_next_token(transition_list, token_first, token_second);
    // printf("FIRST: '%s', CURRENT: '%s', GENERATED'%s'\n", token_first->name, token_second->name, ret->name);

    // Token** tokens = generate_tokens(transition_list, token_first, token_second);
    // printf("for i ");
    // for(int i = 0; i < 4; i++) {
