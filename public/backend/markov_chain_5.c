...
typedef struct {
    Token* previous4; // The fourth previous token in the sequence
...
void update_transition_count(TransitionList* transition_list, Token* previous1, Token* previous2, Token* previous3, Token* previous4, Token* source, Token* destination) {
    for (int i = 0; i < transition_list->length; i++) {
        ...
        if (strcmp(transition->previous1->name, previous1->name) == 0 &&
            ...
            strcmp(transition->previous4->name, previous4->name) == 0 &&
    ...
    new_transition->previous4 = previous4;
...
void train_model(TransitionList* transition_list, TokenList* token_list, Token* dummy, Token* end_token) {    
    ...
    for (int i = 0; i < token_list->length; i++) {
        ...
        if (previous2 != NULL) {
            ...
            update_transition_count(transition_list, previous1, previous2, previous3, previous4, current, destination);
        }
    ...
    update_transition_count(transition_list, previous1, previous2, previous3, previous4, current, end_token);
}
...
void save_transition_list(TransitionList* transition_list, const char* filename) {
    ...
    for (int i = 0; i < transition_list->length; i++) {
        ...
        int length = strlen(transition->previous4->name) + 1;
        fwrite(&length, sizeof(int), 1, fp);
        fwrite(transition->previous4->name, sizeof(char), length, fp);

        length = strlen(transition->previous3->name) + 1;
        fwrite(&length, sizeof(int), 1, fp);
...
Token* generate_next_token(TransitionList* transition_list, Token* previous1, Token* previous2, Token* previous3, Token* previous4, Token* source) {
...
Token** generate_tokens(TransitionList* transition_list, Token* previous1, Token* previous2, Token* previous3, Token* previous4, Token* source) {
    ...
    tokens[0] = generate_next_token(transition_list, previous1, previous2, previous3, previous4, source);
    tokens[1] = generate_next_token(transition_list, previous2, previous3, previous4, source, tokens[0]);
    tokens[2] = generate_next_token(transition_list, previous3, previous4,  source, tokens[0], tokens[1]);
    tokens[3] = generate_next_token(transition_list, previous4, source, tokens[0], tokens[1], tokens[2]);
    tokens[4] = generate_next_token(transition_list, source, tokens[0], tokens[1], tokens[2], tokens[3]);
    tokens[5] = generate_next_token(transition_list, tokens[0], tokens[1], tokens[2], tokens[3], tokens[4]);
    tokens[6] = generate_next_token(transition_list, tokens[1], tokens[2], tokens[3], tokens[4], tokens[5]);
    tokens[7] = generate_next_token(transition_list, tokens[2], tokens[3], tokens[4], tokens[5], tokens[6]);
    tokens[8] = generate_next_token(transition_list, tokens[3], tokens[4], tokens[5], tokens[6], tokens[7]);
    tokens[9] = generate_next_token(transition_list, tokens[4], tokens[5], tokens[6], tokens[7], tokens[8]);
...
int main(int argc, char const *argv[]) {
    ...
    save_transition_list(transition_list, "model_5.bin");
    ...
    // Token* token_five = (Token*) malloc(sizeof(Token));
    // token_five->name = (char*) malloc((strlen(five) + 1) * sizeof(char));
    // strcpy(token_five->name, five);
    // token_five->is_end = 0;

    // Token** tokens = generate_tokens(transition_list, token_first, token_second, token_third, token_four, token_five);
    // printf("for i in range ( ");
    // for(int i = 0; i < 10; i++) {
